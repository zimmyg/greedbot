...
var Game = function (bot) {
    ...
    this.bot.on("presence", this.handlePresence.bind(this));
    this.bot.on("any", this.handleAny.bind(this));
...
Game.prototype.handleMessage = async function(user, userID, channelID, message, event) {
    ...
    if (message.substring(0, 1) === '!') {
        ...
        switch(cmd) {
            ...
            case 'init':
                await this.handleInitCommand(user, userID, channelID, args);
                break;

            case 'quit':
                await this.handleQuitCommand(user, userID, channelID, args);
                break;

            case 'roll':
                await this.handleRollCommand(user, userID, channelID, args);
                break;

            case 'hold':
                await this.handleHoldCommand(user, userID, channelID, args);
                break;

            case 'bank':
                await this.handleBankCommand(user, userID, channelID, args);
                break;

            case 'score':
                await this.handleScoreCommand(user, userID, channelID, args);
                break;

            case 'order':
                await this.handleOrderCommand(user, userID, channelID, args);
                break;
        }
    }
};

Game.prototype.handleScoreCommand = async function(user, userID, channelID, args) {
...
Game.prototype.handleOrderCommand = async function(user, userID, channelID, args) {
...
Game.prototype.handleQuitCommand = async function(user, userID, channelID, args) {
    this.game = null;
    await this.sendMessage({
        to: channelID,
...
Game.prototype.handleBankCommand = async function(user, userID, channelID, args) {
    // the user is attempting to take what score they have held and then move to the next round
    if (!(await this.doCommonChecks(user, userID, channelID))) {
        return;
    }
    ...
    await this.handleNextRoundCommand(user, userID, channelID, args);
};

Game.prototype.handleHoldCommand = async function(user, userID, channelID, args) {
...
Game.prototype.checkPreReqs = async function(channelID) {
...
Game.prototype.handleInitCommand = async function(commandUser, commandUserID, commandChannelID, commandArgs) {
    if (!(await this.checkPreReqs(commandChannelID))) {
        return;
    }
...
Game.prototype.doCommonChecks = async function(user, userID, channelID) {
    if (!this.checkGameRunning()) {
        await this.sendMessage({
            to: channelID,
            message: "There is no game currently running, type !init to start a new game!"
...
Game.prototype.handleRollCommand = async function(user, userID, channelID, args) {
    if (!(await this.doCommonChecks(user, userID, channelID))) return;
    ...
    if (!anyScoringDice) {
        ...
        await this.handleNextRoundCommand(user, userID, channelID, null);
    }
};

Game.prototype.handleNextRoundCommand = async function(user, userID, channelID, args) {
    if (!(await this.doCommonChecks(user, userID, channelID))) return;
    ...
    if ((await this.checkGameFinished(channelID))) return;
...
Game.prototype.checkGameFinished = async function(channelID) {
...
Game.prototype.sendMessage = async function(options) {
    return new Promise((resolve, reject) => {
        this.bot.sendMessage(options, (options) => {
            resolve();
        });
    });
}
